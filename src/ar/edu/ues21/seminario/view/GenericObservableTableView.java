...
import ar.edu.ues21.seminario.repository.ObservableRepository;
...
public class GenericObservableTableView<T, ID> implements ObservableRepository.DataChangeListener<T> {
    private TableView<T> tableView;
    private Class<T> type;
    private GenericRepository<T, ID> repository;
    public GenericObservableTableView(Class<T> type, ObservableRepository<T, ID> repository) {
        this.type = type;
        this.repository = repository;
        this.tableView = new TableView<>();
        repository.addDataChangeListener(this);
    }
    /**
    ...
    @Override
    public void onDataChanged(List<T> newData) {
        Platform.runLater(() -> {
            ObservableList<T> observableList = FXCollections.observableArrayList(newData);
            tableView.setItems(observableList);
        });
    }
    public void setupAcciones(Consumer<T> onEdit, Consumer<T> onDelete) {
